...
number_of_people <- function(results, inp.title, col.pal){ #,  legend_x, legend_y, folder = "plots/tmp",save
...
number_of_people_area_plot_orig_value <- function(res.t, inp.title, col.pal){
  # res.t <- abund.log
  # inp.title <- "Shigella"
  # col.pal <- col.pall.s
  ...
  colnames(pl.table.m) <- c("Ticks", "Agent_amount", "Agent_classes")
  
  pl.table.m[is.na(pl.table.m)] <- 0 

  pl <- ggplot(pl.table.m, aes(x=Ticks, y=Agent_amount, fill=Agent_classes)) + 
    ylab("Number of people in states")+ xlab("Time, days")+
...
trans_number_area_plot_orig_value <- function(res.t, inp.title, col.pall, note.t){
  # res.t <- trans.summary
  # inp.title <- "Shigella"
  ...
  colnames(pl.table.m) <- c("Ticks","trans_val","trans_type")
  pl <- ggplot(pl.table.m, aes(x=Ticks, y=trans_val, fill=trans_type)) +
    ylab("Transitions number per day")+ xlab("Time, days")+
...
trans_number_area_plot <- function(res.t, inp.title, col.pall, note.t){
  ...
  pl <- ggplot(pl.table.m, aes(x=Ticks, y=prop, fill=trans_type)) +
    ylab("Proportion from the total transitions per day")+ xlab("Time, days")+
    ggtitle(paste("Agents transitions between states for",inp.title,sep=" ")) +
    theme(plot.title = element_text(hjust = 0.5),axis.title=element_text(size=13),legend.title=element_text(size=13), legend.text=element_text(size=11), legend.position="bottom",legend.direction="vertical")+
    geom_area(alpha=0.6 , size=1) +
    scale_fill_manual(values=col.pall, name="Transitions type",
